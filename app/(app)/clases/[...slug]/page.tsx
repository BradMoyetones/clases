...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const legal = getSingleContent(["clases", "WEB"], slug);
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                        <div className="mx-auto flex w-full max-w-2xl min-w-0 flex-1 flex-col gap-8 py-6 md:px-0 lg:py-8">
                            <div className="flex flex-col gap-2">
                                <div className="flex flex-col gap-2">
                                    <div className="flex flex-col gap-2 items-start justify-between">
                                        ...
                                        <div className="docs-nav bg-background/80 border-border/50 fixed inset-x-0 bottom-0 isolate z-50 flex items-center gap-2 border-t px-6 py-4 backdrop-blur-sm sm:static sm:z-0 sm:border-t-0 sm:bg-transparent sm:px-0 sm:pt-1.5 sm:backdrop-blur-none">
                                            <PreviewModal src={`/api/clases/${legal.metadata.id}/index.html`} />
                                            <DocsCopyPage
                                                page={legal.content}
                                            />
                                        </div>
                                ...
                                {legal.metadata.languages && legal.metadata.languages.length > 0 && (
                                    <div className="flex flex-wrap gap-2 mt-4">
                                        {legal.metadata.languages.map((lang) => {
